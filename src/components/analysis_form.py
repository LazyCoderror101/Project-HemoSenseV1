...
def get_report_contents(report_source):
    if report_source == "Upload PDF":
        ...
        if uploaded_file:
            ...
            if isinstance(pdf_contents, str) and pdf_contents.startswith(
                (
                    "File size exceeds",
                    "Invalid file type",
                    "Error",
                    "The uploaded file",
                    "abc", # keeping existing structure if needed, but defining new list
                    "Extracted text is too short",
                    "Could not extract",
                    "PDF exceeds",
                    "Password must" # from validators.py just in case
                )
            ):
